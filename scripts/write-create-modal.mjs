import fs from 'fs';
const D = '__D__';
const out = `import React, { useState, useMemo, useEffect } from 'react';
...
export function CreateRequestModal({ open, onClose, onPublished }: Props) {
  ...
  return (
    <${D} className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-200" onClick={handleClose}>
      <${D} className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col transform transition-all animate-in zoom-in-95 duration-200 max-h-[min(92dvh,900px)]" onClick={(e) => e.stopPropagation()}>
        <${D} className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
        ...
        <${D} className="px-6 pt-5 pb-2 shrink-0">
          <${D} className="flex items-center justify-between relative">
            <${D} className="absolute top-1/2 left-0 right-0 h-1 bg-gray-100 -translate-y-1/2 rounded-full" />
            <${D}
            ...
            {STEPS.map((s, idx) => {
              ...
              return (
                <${D} key={s} className="relative z-10 flex flex-col items-center">
                  <${D} className={'w-8 h-8 rounded-full flex items-center justify-center text-sm ' + (isActive ? 'bg-blue-600 text-white shadow-md' : 'bg-white border-2 border-gray-200 text-gray-400')}>
        ...
        <${D} className="p-6 overflow-y-auto overscroll-contain flex-1 min-h-0">
          {step === 'category' && !selectedCategory && (
            <${D} className="animate-in fade-in duration-300">
              ...
              <${D} className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {SERVICE_CATEGORIES.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      <${D} className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center mb-3">
          ...
          {step === 'description' && (
            <${D} className="space-y-4 animate-in fade-in duration-300">
        ...
        <${D} className="px-6 py-4 bg-white border-t border-gray-100 flex justify-between items-center shrink-0">
          {step === 'category' && !selectedCategory ? (
            <span />
          ) : (
...
`;
const path = 'C:/Users/Leandro/Downloads/linkhelp/src/components/client/create-request/CreateRequestModal.tsx';
fs.writeFileSync(path, out.split(D).join('div'));
