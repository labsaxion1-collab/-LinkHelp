...
export function parseAdminDashboardSummary(value: unknown): AdminDashboardSummary | null {
  ...
  if ([totalRequests, openRequests, inProgressRequests, totalApplications, pendingApplications, hiredApplications, hireRate].some((entry) => entry == null)) return null;
  ...
  for (const entry of raw.categories) {
    ...
    if (typeof category.category !== 'string' || !category.category || open == null || applications == null || hired == null || rate == null || (category.average_budget != null && average == null)) return null;
    categories.push({ category: category.category, openRequests: open, applications, hiredApplications: hired, hireRate: rate, averageBudget: average });
  }
  return { totalRequests: totalRequests!, openRequests: openRequests!, inProgressRequests: inProgressRequests!, totalApplications: totalApplications!, pendingApplications: pendingApplications!, hiredApplications: hiredApplications!, hireRate: hireRate!, categories };
}
