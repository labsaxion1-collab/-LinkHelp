...
export function HelperTermsGateModal({ open, onReject, onConfirm, loading }: Props) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="linkhelp-terms-title"
      onClick={onReject}
    >
      <div
      ...
      >
        <h2 id="linkhelp-terms-title" className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">
          {t('auth.terms_modal_title')}
        </h2>
        <p className="mt-2 text-sm text-slate-600 leading-relaxed">{t('auth.terms_modal_sub')}</p>
        ...
        <div className="mt-8 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          <button
            ...
            onClick={onReject}
            disabled={loading}
            className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50"
          >
            {t('auth.terms_reject')}
          </button>
          <button
