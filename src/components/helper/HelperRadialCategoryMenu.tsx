...
function HelperRadialCategoryMenuInner({
  ...
  return (
    <div ref={rootRef} className="relative z-20 mb-2 flex min-h-[52px] items-start gap-2">
      <motion.button
        type="button"
        aria-expanded={open}
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="pointer-events-none absolute left-0 top-0 z-40 h-44 w-44"
          >
            <motion.button
              type="button"
            ...
            {ordered.map((cat, i) => {
              ...
              return (
                <motion.button
                  key={cat.id}
                  type="button"
                  initial={{ scale: 0, opacity: 0, x: 24, y: 24 }}
                  animate={{ scale: 1, opacity: 1, x: 24 + pos.x, y: 24 + pos.y }}
                  exit={{ scale: 0, opacity: 0, x: 24, y: 24 }}
