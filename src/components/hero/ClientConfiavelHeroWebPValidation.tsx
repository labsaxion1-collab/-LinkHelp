...
import { HeroPictureLayer, heroMedalPedestalSrc } from '@/components/hero/HeroPictureLayer';
...
import { CLIENT_CONFIAVEL_HERO_MEDIA } from '@/gamification/hero/clientConfiavelHeroMedia';
...
export function ClientConfiavelHeroWebPValidation({ balance }: Props) {
  const visual = CLIENT_CONFIAVEL_LEVEL_VISUAL;
  const displayBalance =
    balance == null ? '—' : Math.max(0, Math.round(balance)).toLocaleString('pt-BR');
  const medalSrc = heroMedalPedestalSrc(CLIENT_CONFIAVEL_HERO_MEDIA.medal);
  const pedestalSrc = heroMedalPedestalSrc(CLIENT_CONFIAVEL_HERO_MEDIA.pedestal);
  ...
  return (
    <section className="relative left-1/2 isolate mb-4 w-[100dvw] max-w-none -translate-x-1/2 overflow-hidden border-0 bg-[#02040a] text-white shadow-none ring-1 ring-blue-500/10 lg:left-auto lg:w-full lg:translate-x-0 lg:rounded-[1.75rem] lg:shadow-[0_22px_58px_rgba(0,7,30,0.32)]">
      <HeroPictureLayer
        layer={CLIENT_CONFIAVEL_HERO_MEDIA.background}
        aria-hidden="true"
      ...
      <HeroPictureLayer
        layer={CLIENT_CONFIAVEL_HERO_MEDIA.particles}
        aria-hidden="true"
...
/** Alias lazy — mesma entry que ClientConfiavelhero para export tests. */
export { ClientConfiavelHeroWebPValidation as ClientConfiavelhero };
