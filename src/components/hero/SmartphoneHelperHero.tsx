...
export function NewHelperHero({ accountType = 'helper', balance, completedServices, connectedProfessionals, rating, satisfactionRate }: Props) {
  ...
  const currentLevel = isClient ? 'Novo Cliente' : 'Novo Helper';
  const nextLevel = isClient ? '2. Cliente Confiável' : '2. Helper Confiável';
    ...
    <section className="relative left-1/2 isolate mb-4 w-[100dvw] max-w-none -translate-x-1/2 overflow-hidden border-0 bg-[#020804] text-white shadow-none lg:left-auto lg:w-full lg:translate-x-0 lg:rounded-[1.75rem] lg:shadow-[0_22px_58px_rgba(0,20,7,0.32)]">
      ...
      <div className="relative z-10 px-3 pb-3 pt-3 sm:px-8 sm:pb-5 sm:pt-5">
        <header className="flex items-center justify-between gap-3">
          <p className="inline-flex min-w-0 items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.13em] text-lime-300 sm:text-xs sm:tracking-[0.18em]">
            <Sparkles className="h-3.5 w-3.5 shrink-0" /> Sua jornada começa aqui
          </p>
        ...
        <div className="mx-auto mt-1 max-w-[25rem] text-center sm:mt-2">
        ...
        <div className="mx-auto mt-3 w-fit rounded-full border border-lime-300/30 bg-lime-400/10 px-5 py-1.5 backdrop-blur-md">
        ...
        <div className="mx-auto w-fit -translate-y-2">
          <span className="inline-flex min-w-[7.5rem] justify-center rounded-full border border-lime-300/35 bg-gradient-to-b from-lime-400 to-green-800 px-4 py-1 text-sm font-black shadow-[0_0_22px_rgba(132,204,22,0.32)] sm:min-w-[9rem] sm:text-base">Nível 1</span>
        </div>

        <p className="mx-auto -mt-1 max-w-[22rem] text-center text-sm font-medium leading-relaxed text-white/68 sm:max-w-[34rem] sm:text-base">
          {journeyDescription}
        </p>

        <div className="mx-auto mt-3 max-w-[45rem] rounded-2xl border border-lime-400/15 bg-black/40 px-3 py-2.5 backdrop-blur-lg sm:px-5">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1"><p className="text-[10px] text-white/65 sm:text-xs">Próximo nível</p><p className="truncate text-xs font-black uppercase text-lime-400 sm:text-base">{nextLevel}</p></div>
