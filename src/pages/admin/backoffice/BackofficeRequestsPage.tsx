...
export default function BackofficeRequestsPage() {
  ...
  return (
    <BackofficePageShell title={t('backoffice.requests_title')} subtitle={t('backoffice.requests_subtitle')}>
      <input
        value={search}
        onChange={(e) => setSearch(e.target.value)}
        placeholder={t('backoffice.search_requests_placeholder')}
      ...
      <BackofficeTableShell empty={!loading && (data?.requests.length ?? 0) === 0}>
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-white/10 text-xs uppercase text-slate-500">
            <tr>
              <th className="px-4 py-3">{t('backoffice.col_title')}</th>
              <th className="px-4 py-3">{t('backoffice.col_status')}</th>
              <th className="px-4 py-3">{t('backoffice.col_client')}</th>
              <th className="px-4 py-3">{t('backoffice.col_apps')}</th>
            </tr>
          </thead>
          <tbody>
            {(data?.requests ?? []).map((r) => (
